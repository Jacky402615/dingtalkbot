...
- [x] **Step 8: PR-Review fix-loop 3/3（终轮）** — 复评 5 项修复：①stop() 经 stopSignals 竞速唤醒挂在 connect deadline 上的各代 supervisor（不陪跑 30s 定时器，退出即注销信号）；②运行期重建失败不再复用带废弃尝试的旧 client——重建环内退避重试 factory 直至成功（AC3 不 wedge 且不重叠，测试改断言经新 client 恢复）；③loadBotEnv 权限漂移读取前收紧 0600、收紧失败响亮拒绝（测试补）；④robot 非 2xx 错误体读取失败保留因（测试补）；⑤invalidate：文件存在但删除失败（如只读父目录）响亮抛错——存在才删（ENOENT 为成功）。Run 全绿（81 tests）后 commit。
- [x] **Step 7: PR-Review fix-loop 2/3** — 复评 6 项修复：①重建移至 alive() 门之后（旧代 supervisor 超时醒来不得改写换代后的 this.client，stop→restart 竞态测试）；②废弃 connect 迟到 settle 后再次 disconnect 清 socket（防泄漏活连接，late-resolve 测试断言 connected=false）；③首次注册等待钳制在剩余 deadline 内（30s 上界不被 registeredWaitMs 突破，快速失败测试）；④缓存结构异常（合法 JSON 缺字段）warn 留痕；⑤mkdir 移入 try（目录不可用降级仅内存，token 仍返回）；⑥凭据指纹改 JSON.stringify 序列化（含冒号凭据对不碰撞，测试补）。Run 全绿（79 tests）后 commit。
- [x] **Step 6: PR-Review 轮 codex kind=pr 评审修复（fix-loop 1/3）** — 8 项：①缓存键升级为 clientId+clientSecret sha256 指纹（同 clientId 换 secret 也不复用，测试补）；②磁盘缓存权限非 0600 / 不可解析 → warn 忽略（#62，测试补）；③迟到回调在入口丢弃（不进 handler 防重复回显，测试补 handler 计数断言）；④connect 超时后一律经 factory 重建 client（迟到 open 不重叠，late-resolve 测试）；⑤运行期重建失败沿用旧 client 续退避（AC3 不 wedge，测试补）；⑥bun.lock 从腾讯镜像重锁到 registry 无关（URL 空 + sha512 锁定），check-dist 增锁文件卫生检查（禁 http/镜像源）；⑦handler 总预算 50s（withDeadline 截断，测试补）；⑧启动清理/body 解析失败留痕。Run: `bun run typecheck && bun test && bun run build && bun run check:dist` 全绿（74 tests）后 commit。
- [x] **Step 5: 第三轮（终轮）评审修复后收口** — ①in-flight 自引用守卫（旧请求 finally 不清新请求槽位）；②token tmp 独占创建（`wx` + 唯一名，planted 文件收不到内容，清理失败留 error）；③disconnect 废弃失败 → 经 factory 重建 client 续用同代（含重建路径关闭 autoReconnect 的回归测试）。全绿（68 tests）后 commit。codex code 评审三轮：r1 7 项 / r2 6 项 / r3 3 项全部修复；r3 后未再跑第 4 轮（预算收口），残余把关交给 PR-Review 阶段的 kind=pr 评审。
