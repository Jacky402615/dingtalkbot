...
- 下载链：`content.downloadCode`（picture 载荷中的 `pictureDownloadCode` 不使用）→ `POST /v1.0/robot/messageFiles/download`（body `{downloadCode, robotCode}`，无需 unionId）→ 临时 `downloadUrl` → 手动逐跳重定向（≤3 跳、每跳 https、拒 localhost/私网与特殊用途字面 IP（IANA v4/v6 特殊用途注册表穷尽：私网段/v4 映射/NAT64/CGNAT 100.64/10/多播/保留段/TEST-NET×3/192.0.0.0/24/6to4 中继/AMGP 192.31.196.0/24/DVMRP 192.52.193.0/24/benchmark 198.18/15/v6 文档段/Teredo/6to4/discard 100::/64/benchmark 2001:2::/48/ORCHID/站点本地 fec0::/10，均按归一化形态）/凭据、下载请求零认证头）→ 流式落盘 `.bot/uploads/YYYY-MM-DD/<uuid>-<清洗名>.<ext>`（tmp+link 原子发布、文件 0600、目录 0700（创建时显式 chmod 防 umask）/写前清 >1h 陈旧 .tmp；文件名字节预算 200 不切代理对）。（CI-verified）
...
- 时序：媒体下载在 dispatch 准入（去重/鉴权/群白名单/命令）之后；agent-session 忙线预检（与 enqueue 同谓词）先于下载；下载失败不 spawn 会话不入队；媒体无文本时 prompt=前缀+注记；文本消息 prompt 组装零变化；richText 提取的文本不做 @ 剥离且数字文本不当应答（媒体配文语义）。（CI-verified）
- prune：30 天（常量）——启动异步一次 + 24h single-flight 定时器，仅认 YYYY-MM-DD 日期目录，lstat 不跟随 symlink，删空目录，逐项容错聚合 warn；随网关关停停止。（CI-verified）
- 可观测性：每媒体消息一条脱敏汇总 info（msgId/msgtype/附件数/ok|oversize|corrupt|limit|failed 分项/字节/时长/exchange 尝试次数——含失败）；失败 warn 结构化（stage/status，URL 只留 host）；日志绝不携带响应体/downloadCode/query/完整 signed URL/token。（CI-verified）
- live 验证清单：p2p 发图/文件/语音/视频各一（确认归档路径与 prompt 引用）；过期 downloadCode 场景（观察一次真实失败路径的聊天内错误）；richText（图+文）群内 @；大文件超限降级；prune 手工种旧文件验证。

## 配置（D2/D4 契约）

- `.bot/config.json` 键（非法值 warn + 默认）：`session_idle_ttl_minutes`(60) · `ai_card_template_id`("") · `card_content_key`("content") · `model`("glm-5.3-flash") · `agent_permission_mode`("bypassPermissions"|"acceptEdits") · `agent_turn_timeout_ms`(600000) · `claude_bin`("claude") · `card_stream_min_interval_ms`(1500) · `card_stream_min_bytes`(64) · `queue_max_per_chat`(10) · `media_max_bytes`(20971520=20 MiB)（CI-verified）。
...
`.env`（0600）· `config.json`（D1 空默认，键留 D2/D3/D4）· `access.json`（D3 生效：`{admin,approved,groups}` 白名单，手工编辑、每消息读盘、fail-closed）· `token.json`（0600 token 缓存，含凭据指纹；invalidate 删除失败会响亮抛错，绝不静默让失效凭据复活）· `state.json`（连接快照）· `sessions/`（D2 会话）· `uploads/YYYY-MM-DD/`（D4 生效：媒体附件，30 天 prune）· `logs/`（JSONL 每 run 一文件 `YYYYMMDD_HHMMSS.log` + latest.log 链接）· `pids/dingtalkbot.pid`。
